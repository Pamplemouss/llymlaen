...
export default function Play() {
    ...
    const x2 = useMotionValue(0); // totalScore progress bar
    var photospheres = [
                                            ...
                                            <div className="w-10/12 m-auto flex flex-col">
                                                ...
                                                <div className="flex justify-center mt-8 lg:mt-12">
                                                    ...
                                                    >
                                                        ...
                                                        <span className="neosans text-base lg:text-lg text-shadow shadow-black/20 tracking-wide italic inline-block skew-x-12 text-slate-100">{ round === gameSystem.maxRounds ? "Results" : "Next round"}</span>
                                                        <motion.div
                                                            initial={{ opacity: 0 }}
                    ...
                    <div className="h-full w-full relative flex flex-col gap-14 justify-center items-center">
                            ...
                            {gameData.current.locations.map((location: any, index: number) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className={`${gameData.current.scores[index] === 100 ? "bg-yellow-300/80 text-shadow shadow-black/50": "bg-slate-800/80 shadow-black/80"} text-xs xl:text-base text-center neosans text-slate-200 py-1 shadow`}>{location.name} - {location.region}</div>
                        ...
                        <div className="relative overflow-hidden rounded py-3 px-7 neosans text-3xl text-slate-100 shadow-inner shadow-black/90 w-9/12 text-center">
                            <div className="absolute top-0 left-0 h-full w-full bg-slate-900 z-[-1]">
                                <div style={{width: 100*(totalScoreHUD!/(gameSystem.total * gameSystem.maxRounds))+"%"}} className="absolute h-full bg-gradient-to-br from-emerald-800 to-emerald-500"></div>
                            </div>
                            <span className="text-shadow shadow-black/50">Total score: <span className="text-right w-16 inline-block">{Math.round(totalScoreHUD!)}</span> / {gameSystem.maxRounds * gameSystem.total}</span>
                        </div>
