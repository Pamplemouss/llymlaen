...
export default function Home() {
    ...
    function Expansion({expansion, disabled}: {expansion: any, disabled: boolean}) {
        ...
        return (
            <motion.div
                ...
                whileHover={{ scale: disabled ? 1 : [1.1, 1] }}
                /* whileTap={{ scale: [1.2, 1] }} */
                transition={{ duration: 0.1 }}
                className={`vignette ${expansion} ${expansions?.includes(expansion) ? "active" : null} ${disabled ? "disabled opacity-50 border-slate-600/70" : "cursor-pointer border-slate-100/70"} border-4 4k:border-8 w-9/12 h-32 4k:h-60 md:w-3/12 md:h-44 -skew-x-12 group duration-100 overflow-hidden shadow-lg shadow-black/50 relative py-4 px-16 rounded-xl`}
            >
            ...
            <div className="relative md:absolute h-full w-full flex flex-col">
                ...
                <div className="relative h-full w-full flex flex-col md:justify-center items-center pt-24 xl:pt-8">
                    ...
                    <div className="flex-wrap justify-center w-full flex md:flex-flow-col md:flex-rows-3 gap-10 md:w-10/12 4k:w-8/12 4k:gap-16">
                        <Expansion expansion="arealmreborn" disabled={false}></Expansion>
                        <Expansion expansion="heavensward" disabled></Expansion>
                        <Expansion expansion="stormblood" disabled></Expansion>
                        <Expansion expansion="shadowbringer" disabled></Expansion>
                        <Expansion expansion="endwalker" disabled></Expansion>
                    </div>
                    ...
                    <Link href="play">
                        <motion.div
                            ...
                            transition={{ duration: 0.1 }}
                            className="mb-10 md:mb-0 block border-4 4k:border-8 border-slate-100/70 mt-20 -skew-x-12 group duration-100 overflow-hidden shadow-lg shadow-black/50 relative uppercase bg-gradient-to-br hover:shadow-yellow-500 hover:from-yellow-700 hover:to-yellow-400 from-gray-800/50 to-gray-700/50 cursor-pointer py-4 px-16 rounded-xl"
                        >
                            <div className="overflow-hidden absolute top-0 left-0 h-full w-full scale-[0.95] rounded-xl">
                            ...
                            <span className="font-neosans text-4xl 4k:text-6xl text-shadow shadow-black/20 tracking-wide italic inline-block skew-x-12 text-slate-200">Play</span>
                            <motion.div
                                initial={{ opacity: 0 }}
