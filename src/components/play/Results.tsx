...
export default function Results() {
    ...
    const tweet = `Just scored ${gameContext.totalScore}/${(gameContext.gameSystem.total * gameContext.gameSystem.maxRounds)} on Eorguessr!`
    ...
    useEffect(() => {
        if (gameContext.ended) {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    htmlToImage.toCanvas(document.getElementById('score')!)
                    .then(function (canvas) {
                        canvas.toBlob(function(blob) {
                            screenshot.current = blob!;
                        })
                        document.getElementById("screenshot")!.innerHTML = `<img src="${canvas.toDataURL()}">`
                        setSnapReady(true)
                    });
                }, 1500)
    ...
    return (
        <div className="h-full w-full relative flex flex-col justify-center items-center">
            ...
            <div className="pt-16 w-full relative flex flex-col justify-center items-center">
                <div id="score" className="w-11/12 relative justify-center items-center flex flex-col gap-4 md:gap-14 py-12 rounded-lg overflow-hidden shadow shadow-black/50">
                    <div className="absolute h-full w-full bg-gradient-to-br from-indigo-800 to-slate-800 z-[-1]">
                        <div className="absolute h-full w-full bg-slate-900 opacity-50"></div>
                    </div>
                    <motion.div
                        variants={container}
                        ...
                        {gameContext.gameData.current.locations.map((location: any, index: number) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={`score font-neosans text-4xl md:text-6xl grow items-center justify-center flex ${gameContext.gameData.current.scores[index] === 100 ? "text-yellow-300" : "text-slate-100"} text-shadow-lg shadow-slate-900`}>{gameContext.gameData.current.scores[index]}</div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: gameContext.gameSystem.maxRounds*0.25 + 2.55, duration: 0.5 }}>
                    <div className="flex gap-4 relative mt-7 items-center justify-evenly">
                        ...
                        <TwitterShareButton url={`\nhttps://eorguessr.com\n\n`} hashtags={["Eorguessr", "FFXIV", "FF14"]} title={tweet}>
                            <div className="duration-200 -skew-x-12 rounded bg-sky-500 hover:bg-sky-400 w-14 shadow shadow-black py-2 text-slate-100 flex justify-center items-center"><i className="fa-brands fa-twitter"></i></div>
                        </TwitterShareButton>
                ...
                <AnimatePresence>
                    { snapshot && (
                        ...
                        >
                            <div className="bg-slate-600 text-slate-100 rounded mb-2 shadow shadow-black p-1 text-center">Screenshot saved in your clipboard!</div>
                            <div className="w-80 border-2 rounded border-slate-400 overflow-hidden">
                                <img src={document.getElementById("screenshot")?.children[0].getAttribute("src")!}></img>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
